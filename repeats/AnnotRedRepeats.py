        ...
        for rfilename in rpt_files:
            ...
            for line in rfile:
                ...
                if chrsequence[idx_start:idx_end] == "":
                    print(
                        "# ERROR: cannot cut seq_region %s:%s-%s"
                    ...
                    outfile.write(
                        ">%s:%s:%s\n%s\n"
                        % (seq_region_id, start, end, chrsequence[idx_start:idx_end])
                    )
...
def format_reference_minimap(miniexe, cores, fasta_file, outdir):
    ...
    if (
        os.path.isfile(formatted_filename)
        and os.path.getsize(formatted_filename) > 0
        and os.path.isfile(log_filepath)
    ):
        try:
        ...
        for line in logfile:
            ...
            if ramre and samelib:
                print(
                    "# re-using previously formatted repeat library ",
                    formatted_filename,
                )
...
def run_minimap(miniexe, cores, lib_filename, fasta_filename, outdir):
    ...
    # check whether previous results exist
    if (
        os.path.isfile(sorted_filename)
        and os.path.getsize(sorted_filename) > 0
        and os.path.isfile(log_filepath)
    ):
...
def make_annotation_report(
    map_filename, log_filename, minlen, bed_filename, seqregion2name, verbose=False
):
    ...
    for line in logfile:
        ...
        if statsre:
            print(
                "# Genome length: %s Repeated content: %s %2.1f%% Annotated: %d %2.1f%%\n"
                % (
                    statsre.group(1),
                    statsre.group(2),
                    100 * int(statsre.group(2)) / int(statsre.group(1)),
                    annotated_length,
                    100 * annotated_length / int(statsre.group(1)),
                )
            )
...
def _annotated_repeats_to_files(
    workdir, matched_repeats, sequences, analysis_id, last_consensus_id
):
...
def store_annotated_repeat_database(
    workdir,
    matched_repeats,
    exe,
    minimap_version,
    ...
    # repeat_result=1
    (repeat_filename, cons_filename) = _annotated_repeats_to_files(
        workdir, matched_repeats, sequences, analysis_id, dummy_consensus_id
    )
...
def main():
    ...
    parser.add_argument("outdir", help="path to directory with stored Red results")
    parser.add_argument(
        "--exe",
        default=default_exe,
    ...
    parser.add_argument(
        "--cor", default=1, help="number of cores for minimap2, default: 1"
    )
    parser.add_argument(
        "--minlen",
        default=90,
    ...
    parser.add_argument(
        "--bed_file", default="", help="name of output BED file with annotated repeats"
    )
    parser.add_argument("--host", help="name of the database host")
    parser.add_argument("--user", help="host user")
    parser.add_argument("--pw", help="host password")
    parser.add_argument("--port", type=int, help="host port")
    parser.add_argument("--db", help="name of the core database")
    parser.add_argument(
        "--logic_name",
        default="repeatdetector_annotated",
    ...
    (repeats_filename, seqregion2name) = fetch_repeats_FASTA(
        log_filepath, syn_filepath, annotdir, int(args.minlen)
    )
    print(
        "# FASTA file with repeat sequences (length>%s): %s\n\n"
    ...
    formatted_lib_filename = format_reference_minimap(
        args.exe, args.cor, args.repeat_fasta_file, annotdir
    )

    # run minimap, or else re-use previous results
    # Note: Red-called repeats are handled as long reads
    print("# running minimap")
    (map_filename, version) = run_minimap(
        args.exe, args.cor, formatted_lib_filename, repeats_filename, annotdir
    )
    print("# mapped repeats: ", map_filename)

    matched_repeats = make_annotation_report(
        map_filename, log_filepath, int(args.minlen), args.bed_file, seqregion2name
    )
    ...
    if args.user and args.pw and args.host and args.port and args.db:
        ...
        num_annot = store_annotated_repeat_database(
            annotdir,
            matched_repeats,
            args.exe,
            version,
            args.repeat_fasta_file,
            args.logic_name,
            description,
            displaylabel,
            db_url,
