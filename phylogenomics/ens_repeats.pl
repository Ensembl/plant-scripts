...
# Downloads sequences of repeated elements of a clade from Ensembl Plants.
#
...
sub help_message {
    print "\nusage: $0 [options]\n\n"
      ...
      . "-m min repeat length               (optional, default: -m $minlen)\n"
      . "-o outgroup species_name           (optional, example: -o brachypodium_distachyon)\n"
      . "-i ignore species_name(s)          (optional, example: -i selaginella_moellendorffii -i ...)\n";

    print "Example calls:\n\n"
      . " perl $0 -c Liliopsida -o arabidopsis_thaliana -f Liliopsoda.Atha.repeats.EG44.fna\n";
    exit(0);
}
...
if ($show_supported) {
    ...
    if ($minlen < 1) {
        print "# ERROR: need a valid min length, such as -m 90\n\n";
		exit;
    }

    print "# $0 -d $division -c $taxonid -o $out_genome -m $minlen -f $outfile\n\n";
}
